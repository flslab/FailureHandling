...
for (( i=1; i<num_of_total_servers; i++ )); do
    ...
    ssh -oStrictHostKeyChecking=no -f "${server_addr}" "ulimit -n 99999 && cd FailureHandling && cp experiments/config$1.py config.py && sleep 10 && sudo nohup python3 secondary.py i> log/$(date '+%Y-%m-%d_%H-%M-%S').log 2>&1 &" &
done
