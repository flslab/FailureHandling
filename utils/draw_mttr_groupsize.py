...
mttr_list = [[],[]]
QoI_list = [[],[]]
for i, filename in enumerate(["pri30", "no30"]):

    for folder in ["K0", "K3", "K5", "K10", "K20"]:
        input_path = f"/Users/shuqinzhu/Desktop/dragon/{folder}/dragon_D1_R10_T60_S6_N{filename}/dragon_D1_R10_T60_S6_N{filename}_final_report.xlsx"
...
plt.figure()
pri_line, = plt.plot([0, 3, 5, 10, 20], mttr_list[0], marker='o', label=f'With Priority Queue')

nopri_line, = plt.plot([0, 3, 5, 10, 20], mttr_list[1], marker='x', label=f'No Priority Queue')
...
plt.figure()
pri_line, = plt.plot([0, 3, 5, 10, 20], QoI_list[0], marker='o', label=f'With Priority Queue')

nopri_line, = plt.plot([0, 3, 5, 10, 20], QoI_list[1], marker='x', label=f'No Priority Queue')
