...
def draw_cube(ax, xs, ys, zs, s=1, c='white', a=1):
    for i in range(len(xs)):
        ...
        ax.add_collection3d(Poly3DCollection(faces, facecolor=c, linewidths=1, edgecolor=c, alpha=a))
...
def update(frame, titles):
    ...
    draw_cube(ax, xs, ys, np.array(zs), s=1, c='white', a=1)
    ...
    draw_cube(ax3, xs, ys, np.array(zs), s=1, c='white', a=1)
    set_wheel(ax3)
    ...
    draw_cube(ax2, xs, ys, np.array(zs), s=1, c='white', a=1)
    set_axis(ax2, length, width, height)
    update_title(ax2, titles[1], total_points - len(coords_K3))

    ax4.clear()

    draw_cube(ax4, xs, ys, np.array(zs), s=1, c='white', a=1)
...
if __name__ == '__main__':
    ...
    for i, file_names in enumerate(file_name_list):
        ...
        ani.save(f"/Users/shuqinzhu/Desktop/video/videos/{video_name_list[i]}_wheel_R1.mp4", writer=writer)
